...
import options
import ros_utils
from detection_lib.yolov5 import utils as gfuncs
...
class snu_module_ros(object):
    def __init__(self, subscribe_rate=10):
        # Initialize Multiple Objects Class
        self.mobjs = MultipleObjects(opts=opts)
    ...
    def run_algorithm(self):
        prev_sync_data_dict = None
        sync_data_dict = None
        # Load Frameworks
        frameworks = {
            "det": load_det_model(opts=opts),
            "acl": load_acl_model(opts=opts),
        }

        # Declare ROS Subscribe Rate
        r = rospy.Rate(self.subscribe_rate)
        while not rospy.is_shutdown():
            prev_sync_data_dict = sync_data_dict
            ...
            if prev_sync_data_dict is not None:
                pass
            ...
            if all_synced is True:
                ...
                self.mobjs, detections, algorithm_time_dict = \
                    snu_algorithms.usr_integrated_snu(
                        fidx=self.fidx, sync_data_dict=sync_data_dict, frameworks=frameworks,
                        multiple_objs=self.mobjs, opts=opts
                    )
                ...
                # # Run Visualizer
                # self.visualizer(sensor_data=vis_data, tracklets=self.mobjs.trks, detections=detections)

                # Publish ROS Message
                # TODO: Write a Publisher Code

            pre_sync_data_dict = copy.deepcopy(sync_data_dict)
...
def main():
    ...
    ros_embedded_snu_module = snu_module_ros(subscribe_rate=1000)
