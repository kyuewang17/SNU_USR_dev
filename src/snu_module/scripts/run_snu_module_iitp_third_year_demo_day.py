...
def visualize_tracklets(draw_rgb_frame, trks, opts, draw_rgb_frame_acl=None):
    for trk in trks:
        ...
        if draw_rgb_frame_acl is not None:
            cv2.rectangle(draw_rgb_frame_acl,
                          (state_bbox[0], state_bbox[1]), (state_bbox[2], state_bbox[3]),
                          (trk.color[0], trk.color[1], trk.color[2]),
                          opts.visualization.tracking['linewidth'])
        ...
        if draw_rgb_frame_acl is not None:
            cv2.rectangle(draw_rgb_frame_acl, box_coords[0], box_coords[1], (trk.color[0], trk.color[1], trk.color[2]), cv2.FILLED)
            # cv2.putText receives "bottom-left" point of the text
            cv2.putText(draw_rgb_frame_acl, trk_id_str, (text_x, text_y), font, font_size,
                        (255 - trk.color[0], 255 - trk.color[1], 255 - trk.color[2]), thickness=2)
        ...
        if trk.pose is not None and opts.visualization.aclassifier['is_draw'] is True:
            if trk.label == 1:
                ...
                cv2.putText(draw_rgb_frame_acl, pose_word, (min(int(trk.x[0] + (trk.x[4] / 2)), W - 1), min(int(trk.x[1] + (trk.x[5] / 2)), H - 1)),
...
def ros_main():
    ...
    # Additional Publisher for IITP Final
    pub_det_img = rospy.Publisher(opts.publish_mesg["detection_result"], Image, queue_size=1)
    pub_trk_img = rospy.Publisher(opts.publish_mesg["tracking_result"], Image, queue_size=1)
    pub_acl_img = rospy.Publisher(opts.publish_mesg["aclassification_result"], Image, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        # IITP Final Demo
        # Copy Each Result Frames
        draw_rgb_frame_2 = copy.deepcopy(draw_rgb_frame)
        draw_rgb_frame_3 = copy.deepcopy(draw_rgb_frame)
        ...
        if opts.visualization.tracking['is_draw'] is True:
            visualize_tracklets(draw_rgb_frame_2, trackers, opts, draw_rgb_frame_acl=draw_rgb_frame_3)
        #################################################################
        ...
        # [2] SNU Result Image
        pub_det_img.publish(bridge2.cv2_to_imgmsg(draw_rgb_frame, "rgb8"))      # Detection
        pub_trk_img.publish(bridge2.cv2_to_imgmsg(draw_rgb_frame_2, "rgb8"))    # Tracking
        pub_acl_img.publish(bridge2.cv2_to_imgmsg(draw_rgb_frame_3, "rgb8"))    # Action
        ########################################################
...
def custom_main():
    ...
    # For Image Sequence,
    for fidx in range(frame_length):
