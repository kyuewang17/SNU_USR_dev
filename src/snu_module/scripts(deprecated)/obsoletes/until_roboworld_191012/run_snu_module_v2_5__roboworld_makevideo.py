...
# import options_kiro_all_bag as options
# import options_roboworld_realtime as options
import options
...
def visualize_detections(draw_frame, detections, opts):
...
def visualize_tracklets(draw_frame, trks, opts):
    for trk in trks:
        ...
        cv2.rectangle(draw_frame, box_coords[0], box_coords[1], (trk.color[0], trk.color[1], trk.color[2]), cv2.FILLED)
        # cv2.putText receives "bottom-left" point of the text
        cv2.putText(draw_frame, trk_id_str, (text_x, text_y), font, font_size,
                    (255 - trk.color[0], 255 - trk.color[1], 255 - trk.color[2]), thickness=2)

        # Visualize Smoothed Camera Coordinate Velocity
        if trk.cam_coord_vel is not None:
            cam_coord_vel = trk.cam_coord_vel
            ...
            cv2.rectangle(draw_frame, box_coords[0], box_coords[1], (trk.color[0], trk.color[1], trk.color[2]), cv2.FILLED)
            # cv2.putText receives "bottom-left" point of the text
            cv2.putText(draw_frame, cam_coord_vel_str, (text_x, text_y), font, font_size * 0.8,
                        (255 - trk.color[0], 255 - trk.color[1], 255 - trk.color[2]), thickness=2)

        # Visualize Smoothed Camera Coordinate Position
        if trk.cam_coord is not None:
            ...
            cv2.rectangle(draw_frame, box_coords[0], box_coords[1], (trk.color[0], trk.color[1], trk.color[2]), cv2.FILLED)
            # cv2.putText receives "bottom-left" point of the text
            cv2.putText(draw_frame, cam_coord_pos_str, (text_x, text_y), font, font_size * 0.8,
                        (255 - trk.color[0], 255 - trk.color[1], 255 - trk.color[2]), thickness=2)
        ...
        if trk.depth is not None:
            ...
            cv2.putText(draw_frame, trk_depth_str, (text_x, text_y), font, 1.2,
                        (255 - trk.color[0], 255 - trk.color[1], 255 - trk.color[2]), thickness=2)


# Visualize Action Classification Result
def visualize_aclassification(draw_frame, trks, opts):
    for trk in trks:
        if trk.label == 1:
            state_bbox, _ = snu_mmt.fbbox.zx_to_bbox(trk.states[-1])
            state_bbox = state_bbox.astype(np.int32)
            ...
            # Visualize Action Classification Result (for Person type only)
            H = draw_frame.shape[0]
            W = draw_frame.shape[1]
            cv2.putText(draw_frame, str(int(trk.pose)), (min(int(trk.x[0] + (trk.x[4] / 2)), W - 1), min(int(trk.x[1] + (trk.x[5] / 2)), H - 1)),
...
def ros_main():
    ...
    pub_input_frame = rospy.Publisher("/osr/snu_input_frame", Image, queue_size=1)

    # Publish for Each Algorithm Sub-modules
    pub_detection_result = rospy.Publisher("/osr/snu_detection_result", Image, queue_size=1)
    pub_tracking_result = rospy.Publisher("/osr/snu_tracking_result", Image, queue_size=1)
    pub_aclassify_result = rospy.Publisher("/osr/snu_aclassify_result", Image, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        ##############################################################
        # Copy D435 RGB Image for Detection Visualization
        raw_rgb_frame = copy.copy(imgStruct_dict['rgb'].frame.raw)
        draw_detection_rgb_frame = copy.copy(imgStruct_dict['rgb'].frame.raw)
        draw_tracking_rgb_frame = copy.copy(imgStruct_dict['rgb'].frame.raw)
        draw_aclassification_rgb_frame = copy.copy(imgStruct_dict['rgb'].frame.raw)
        ...
        print(detections['confs'].reshape((1, -1)))
        ############################################
        ...
        if opts.visualization.detection['is_draw'] is True:
            visualize_detections(draw_detection_rgb_frame, detections, opts)
        # [2] Tracking and Action Classification Result Visualizer
        if opts.visualization.tracking['is_draw'] is True:
            visualize_tracklets(draw_tracking_rgb_frame, trackers, opts)
        # [3] Action Classification Result Visualizer
        if opts.visualization.aclassifier['is_draw'] is True:
            visualize_aclassification(draw_aclassification_rgb_frame, trackers, opts)
        #################################################################
        ...
        # Publish Each Sub-modules
        pub_input_frame.publish(bridge2.cv2_to_imgmsg(raw_rgb_frame, "rgb8"))
        if opts.visualization.detection['is_draw'] is True:
            pub_detection_result.publish(bridge2.cv2_to_imgmsg(draw_detection_rgb_frame, "rgb8"))
        if opts.visualization.tracking['is_draw'] is True:
            pub_tracking_result.publish(bridge2.cv2_to_imgmsg(draw_tracking_rgb_frame, "rgb8"))
        if opts.visualization.aclassifier['is_draw'] is True:
            pub_aclassify_result.publish(bridge2.cv2_to_imgmsg(draw_aclassification_rgb_frame, "rgb8"))
        ########################################################
        ...
        if opts.visualization.detection['is_draw'] is True or opts.visualization.tracking['is_draw'] is True:
            ...
            if fidx == 1:
                cv2.moveWindow(winname, imshow_x, imshow_y)
            # RGB to BGR (for bag files from agent)
            cv2.imshow(winname, cv2.cvtColor(draw_tracking_rgb_frame, cv2.COLOR_RGB2BGR))
