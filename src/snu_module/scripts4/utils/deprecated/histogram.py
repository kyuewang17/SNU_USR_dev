...
# import numba
import numpy as np
from scipy.ndimage import histogram as scipy_histogram
...
def histogramize_patch(sensor_patch, dhist_bin, min_value, max_value):
    if sensor_patch is [] or (sensor_patch.shape[0] == 0 or sensor_patch.shape[1] == 0):
        ...
        if patch_max_value < 0:
            ...
            # hist, idx = scipy_histogram(sensor_patch_vec, dhist_bin, min_val=min_value, max_val=max_value)
            hist, idx = scipy_histogram(sensor_patch, min=min_value, max=max_value, bins=dhist_bin)
    return hist, idx