...
class RefineDetPostProc(RefineDetPostProcBase):
    ...
    def detect(self, odm_predictions, refined_anchors, ignore_flags_refined_anchor):
        ...
        for idx in range(num):
            ...
            # add
            keep_idxes = torch.nonzero(
                (float(self.max_w)/self.input_w) >(cls_boxes_s[0,:,2] - cls_boxes_s[0,:,0] )).view(-1)
            cls_boxes_s = cls_boxes_s[:, keep_idxes]
            cls_confs_s = cls_confs_s[:, keep_idxes]
            #
