...
if os.name == "nt":
    ...
    lib = CDLL(cur_path + "/build_release/libdark.so", RTLD_GLOBAL)
    # lib = CDLL(cur_path+"/libdarknet.so", RTLD_GLOBAL)
...
do_nms_sort = lib.do_nms_sort_custom
...
def detect_image(net, meta, im, thresh=.5, hier_thresh=.5, nms=.45, debug= False):
    ...
    altNames = ["person", "bike", "car", "bike", " ", "car", " ", "car"]
...
def return_net(configPath = cur_path+"/cfg/yolov4.cfg",\
    weightPath = cur_path+"/yolov4.weights",batch_size=1):
...
def inference_(net, custom_image_bgr, thresh= 0.25, metaPath= cur_path+"/cfg/coco.data",\
    hier_thresh=.5, nms=.45, batch_size=1):
    # img_samples = ['data/person.jpg', 'data/person.jpg', 'data/person.jpg']
    # image_list = [cv2.imread(k) for k in img_samples]
    
    meta = load_meta(metaPath.encode('utf-8'))
    custom_image_bgr = custom_image_bgr[0].permute(1,2,0).cpu().numpy()
    image_list = [custom_image_bgr]
    pred_height, pred_width, c = custom_image_bgr.shape
    net_width, net_height = (network_width(net), network_height(net))
    img_list = []
    
    custom_image = custom_image_bgr
    # custom_image = cv2.cvtColor(custom_image_bgr, cv2.COLOR_BGR2RGB)
    custom_image = cv2.resize(
        custom_image, (net_width, net_height), interpolation=cv2.INTER_NEAREST)
    ...
    for b in range(1):
        ...
        for i in range(num):
            ...
            if score > thresh:
                ...
                boxes.append(torch.tensor([left, top, right, bottom]).cuda().unsqueeze(0))
        ...
        result_dict = dict()
        result_dict['confs_l'] = scores
        result_dict['labels_l'] = classes
        result_dict['boxes_l'] = boxes
        return result_dict
