...
class visualizer(object):
    ...
    def call_old(self, sensor_data, trajectories, detections, fidx, _check_run_time=False, segmentation=None):
        ...
        if self.vopts.tracking["is_draw"] is True:
            ...
            if self.vopts.tracking["auto_save"] is True:
                auto_save_trajectory_base_dir = \
                    os.path.join(os.path.dirname(os.path.dirname(__file__)), "sample_results", "trajectory")
                if os.path.isdir(auto_save_trajectory_base_dir) is False:
                    os.mkdir(auto_save_trajectory_base_dir)

                if len(trajectories) != 0:
                    self.save_frame(
                        save_base_dir=auto_save_trajectory_base_dir, frame=trk_acl_frame,
                        fidx=fidx, modal=sensor_data.get_modal_type()
                    )
        else:
    ...
    def visualize_top_view_trajectories(self, trajectories):
        ...
        top_view_winname = "Top-View Trajectory Results"

        # For Drawing Top-view Trajectories
        ...
        # Draw Trajectory Coordinates to Top-view Map
        for trk_idx, trk in enumerate(trajectories):
            # Trajectory Coordinates < (-y) and (+x) Coordinates >
