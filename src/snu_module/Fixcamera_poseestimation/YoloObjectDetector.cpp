...
void YoloObjectDetector::init()
{
  ...
  // Tracking Analyzer init
  tracker.initialize("OSRfix1_cam", "TrackingAnalyzer.yml");
  ...
  std::string trackingObjTopicName;
  int trackingObjQueueSize;
  bool trackingObjLatch;
  ...
  nodeHandle_.param("publishers/tracking_obj/topic", trackingObjTopicName,
                    std::string("tracking_obj"));
  nodeHandle_.param("publishers/tracking_obj/queue_size", trackingObjQueueSize, 1);
  nodeHandle_.param("publishers/tracking_obj/latch", trackingObjLatch, false);
  ...
  detectionImagePublisher_ = nodeHandle_.advertise<sensor_msgs::Image>(detectionImageTopicName,
                                                                       ...
                                                                       detectionImageLatch);
  trackingObjPublisher_ = nodeHandle_.advertise<osr_msgs::Tracks>(
      trackingObjTopicName, trackingObjQueueSize, trackingObjLatch);
...
void *YoloObjectDetector::publishInThread()
{
  ...
  std::vector<bbox_t> yolo_detections;  
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          yolo_detect.obj_id = i;
          boundingBox.probability = rosBoxes_[i][j].prob;
          yolo_detect.prob = rosBoxes_[i][j].prob;
          boundingBox.xmin = xmin;
          boundingBox.ymin = ymin;
          boundingBox.xmax = xmax;
          boundingBox.ymax = ymax;
          yolo_detect.x = rosBoxes_[i][j].x;
          yolo_detect.y = rosBoxes_[i][j].y;
          yolo_detect.w = rosBoxes_[i][j].w;
          yolo_detect.h = rosBoxes_[i][j].h;
          boundingBoxesResults_.bounding_boxes.push_back(boundingBox);
          yolo_detections.push_back(yolo_detect);
        }
      }
    }
  ...
  // Tracking Analyzer update
  double image_acquisition_time_sec = what_time_is_it_now(); // / 1000.0;
  tracker.update(yolo_detections, image_acquisition_time_sec);

  // tracking result
  std::vector<obj_t> objs = tracker.getResult();

  for (int i = 0; i < objs.size(); i++) {
    osr_msgs::Track track;
    osr_msgs::BoundingBox boundingBox;

    track.obj_id = objs[i].obj_id;
    track.class_id = objs[i].class_id;
    track.obj3dx = objs[i].x;
    track.obj3dy = objs[i].y;
    track.obj3dvx = objs[i].vx;
    track.obj3dvy = objs[i].vy;
    boundingBox.x = objs[i].ix;
    boundingBox.y = objs[i].iy;
    boundingBox.width = objs[i].iw;
    boundingBox.height = objs[i].ih;
    track.bbox_pose = boundingBox;
    trackResults_.tracks.push_back(track);

    trackResults_.header.stamp = ros::Time::now();
    trackResults_.header.frame_id = "tracking";
    trackingObjPublisher_.publish(trackResults_);
  }
